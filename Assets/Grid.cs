...
public class Grid : MonoBehaviour
{
    ...
    private void Start()//Ran once the program starts
    {
        ...
        var tempiGridSizeX = vGridWorldSize.x / fNodeDiameter;//Divide the grids world co-ordinates by the diameter to get the size of the graph in array units.
        var tempiGridSizeY = vGridWorldSize.y / fNodeDiameter;//Divide the grids world co-ordinates by the diameter to get the size of the graph in array units.

        iGridSizeX = Mathf.RoundToInt(tempiGridSizeX);//Divide the grids world co-ordinates by the diameter to get the size of the graph in array units.
        iGridSizeY = Mathf.RoundToInt(tempiGridSizeY);//Divide the grids world co-ordinates by the diameter to get the size of the graph in array units.
    ...
    void CreateGrid()
    {
        ...
        for (int x = 0; x < iGridSizeX; x++)//Loop through the array of nodes.
        {
            ...
            {
                worldPoint = bottomLeft + Vector3.right * (x * fNodeDiameter + fNodeRadius) + Vector3.forward * (y * fNodeDiameter + fNodeRadius);//Get the world co ordinates of the bottom left of the graph
                castPoint = worldPoint + Vector3.up * (10);
                RaycastHit hit;
                if (Physics.Raycast(castPoint, -Vector3.up, out hit, hitLayers))
                {
                    print("Hit at " + hit.point.y);
                    worldPoint = new Vector3(worldPoint.x, hit.point.y, worldPoint.z);
                    centerPoint = new Vector3(worldPoint.x + fNodeRadius, worldPoint.y, worldPoint.z);
                }
                ...
                NodeArray[x, y] = new Node(Wall, worldPoint, centerPoint, x, y);//Create a new node in the array.
            }
        }
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        if (NodeArray != null)//If the grid is not empty
        {
            foreach (Node n in NodeArray)//Loop through every node in the grid
            {
                ...
                if (n.bIsStart || n.bIsEnd)//If the current node is a start or end block
                {
